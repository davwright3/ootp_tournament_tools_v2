...
def test_basic_batting_stats_return_correct_values(patched_data_store):
    """
...
def test_returns_only_eligible_cards(patched_data_store):
...
def test_returns_only_selected_stats(patched_data_store):
...
def test_returns_only_higher_than_min_pa(patched_data_store):
    df = mod.calc_basic_batting_stats_df(min_pa=45, cull_team_limit_select=12)

    assert 73691 in df['CID'].tolist()
    assert 73885 not in df['CID'].tolist()

def test_returns_cards_within_selected_value_range(patched_data_store):
